...
sdmt_norms <- function(education, age, male=TRUE,
  ...
  data <- read_excel("../Database/SDMT_Norms.xlsx", sheet=source, col_names=FALSE)
...
.compute_sdmt_scores <- function(reference, source,
  ...
  if (!is.na(written)) {
    written_mean = reference[str_detect(colnames(reference), "(written).+(mean)|(value).+(mean)")]
    written_sd = reference[str_detect(colnames(reference), "(written).+(sd)|(value).+(sd)")]
  ...
  # Verbal
  if (!is.na(verbal)) {
    verbal_mean = reference[str_detect(colnames(reference), "(verbal).+(mean)")]
    verbal_sd = reference[str_detect(colnames(reference), "(verbal).+(sd)")]
